...
data = data[data.day_from_admission.isin([0, 1, 2, 3, 4])]
...
# Aggregate numbers
aux = data.groupby(['study_no', 'outcome'])[features] \
    .agg(functions)
...
data = aux.merge(data[['age', 'gender', 'study_no']].groupby('study_no').head(1), how='left', on='study_no')
print(data.shape)
print(data.columns)
